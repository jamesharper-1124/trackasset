...
<head>
    <link rel="stylesheet" href="{{ asset('css/add_rooms.css') }}?v={{ time() }}">
    <script src="https://code.jquery.com/jquery-3.6.0.min.js"></script>
    <script src="{{ asset('js/dashboard.js') }}?v={{ time() }}"></script>
    <script src="{{ asset('js/edit_rooms.js') }}?v={{ time() }}"></script>
...
<body>
        ...
        <div class="main-wrapper">
            ...
            <main class="content-area">
                <div class="form-container">
                    ...
                    <form action="{{ route('rooms.update', $room->id) }}" method="POST" enctype="multipart/form-data" data-mode="edit" data-id="{{ $room->id }}">
                        @csrf
                        @method('PUT')
                        @if ($errors->any())
                            <div
                        ...
                        <div class="form-grid">
                            ...
                            <div class="form-group" style="display: flex; flex-direction: column; align-items: center;">
                                <label for="room_photo" class="image-upload-wrapper" style="cursor: pointer;">
                                    <img src="{{ asset($room->room_photo ?? 'images/rooms/default.png') }}"
                                    ...
                                    <input type="file" name="room_photo" id="room_photo" class="file-input-hidden"
                            ...
                            <div style="display: flex; flex-direction: column; gap: 1.5rem;">
                                <div>
                                    ...
                                    <input type="text" name="room_name" class="form-input"
                                ...
                                <div>
                                        ...
                                        <select id="user-select" class="form-select">
                                            ...
                                            @foreach($users as $user)
                                                <option value="{{ $user->id }}"
                                                    data-image="{{ asset($user->profile_photo) }}"
                                                    data-name="{{ $user->firstname }} {{ $user->lastname }}"
                                                    {{ $room->managers->contains($user->id) ? 'style=display:none' : '' }}>
                                    ...
                                    <div id="selected-users-container"
                                        ...
                                        <!-- Pre-populate existing managers -->
                                        @foreach($room->managers as $manager)
                                            <div class="user-tag" style="display: flex; align-items: center; gap: 0.5rem; background-color: #e5e7eb; padding: 0.25rem 0.5rem; border-radius: 9999px; font-size: 0.875rem;">
                                                <img src="{{ asset($manager->profile_photo) }}" alt="{{ $manager->firstname }} {{ $manager->lastname }}" style="width: 1.5rem; height: 1.5rem; border-radius: 50%; object-fit: cover;">
                                                <span>{{ $manager->firstname }} {{ $manager->lastname }}</span>
                                                <button type="button" class="remove-user-btn" data-id="{{ $manager->id }}" style="background: none; border: none; cursor: pointer; color: #6b7280; font-size: 1rem; line-height: 1;">&times;</button>
                                            </div>
                                        @endforeach
                                    </div>
                                    <!-- Hidden inputs for existing managers -->
                                    <div id="hidden-inputs">
                                        @foreach($room->managers as $manager)
                                            <input type="hidden" name="managed_by[]" value="{{ $manager->id }}" id="input-user-{{ $manager->id }}">
                                        @endforeach
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="form-actions">
                            <a href="{{ route('rooms') }}" class="btn-cancel">Cancel</a>
    ...
    <!-- Inline script moved to public/js/add_rooms.js -->
