...
<head>
    ...
    <title>Inventories in {{ $room->room_name }} - TrackAsset</title>
    <!-- Link to custom CSS -->
    <link rel="stylesheet" href="{{ asset('css/inventories.css') }}?v={{ time() }}">
    <script src="{{ asset('js/dashboard.js') }}?v={{ time() }}"></script>
    <script src="{{ asset('js/view_inventory.js') }}?v={{ time() }}"></script>
        ...
        <div class="main-wrapper">
            ...
            <header class="top-header">
                ...
                <div class="header-left">
                    ...
                    <h2 class="page-title">Inventories in {{ $room->room_name }}</h2>
                </div>
                    ...
                    <div class="controls-top">
                        ...
                        <div class="search-section">
                            ...
                            <div class="filter-radios">
                                ...
                                <label class="radio-label">
                                    <input type="radio" name="status-filter" value="GOOD"> Good
                                </label>
                                <label class="radio-label">
                                    <input type="radio" name="status-filter" value="NEEDS ATTENTION"> Needs Attention
                                </label>
                                <label class="radio-label">
                                    <input type="radio" name="status-filter" value="N.G"> N.G.
                                </label>
                            </div>
                        </div>
                    </div>
                ...
                <div class="inventory-section">
                    <div class="horizontal-list">
                        @forelse($inventories as $inventory)
                            <div class="h-card" data-status="{{ $inventory->status_condition }}">
                                <!-- Left: Image -->
                                <div class="h-card-img-wrapper">
                                    <img src="{{ asset($inventory->inventory_photo ?? 'images/inventory/default.png') }}"
                                        alt="{{ $inventory->inventory_name }}">
                                </div>

                                <!-- Right: Content Wrapper -->
                                <div class="h-card-content">
                                    <div class="h-card-info">
                                        <h4 class="h-card-title">{{ $inventory->inventory_name }}</h4>

                                        @php
                                            $statusLabel = $inventory->status_condition;
                                        ...
                                        <div class="status-wrapper" style="margin-top: 0.5rem;">
                                            <span class="card-status {{ $statusClass }}">{{ $statusLabel }}</span>
                                        </div>
                                        <div class="remarks-wrapper" style="margin-top: 0.25rem;">
                                            <span class="remarks-text">Remarks: {{ $inventory->remarks ?? 'NONE' }}</span>
                                        </div>
                                        <div style="font-size: 0.75rem; color: #9ca3af; margin-top: 0.25rem;">
                                            Updated: {{ $inventory->updated_at->format('M d, Y') }}
                                        </div>
                                    </div>

                                    <!-- Although we don't have direct actions here (Use Rooms > Edit), we can display dates or generic info if needed -->
                                </div>
                            </div>
                        @empty
                            <div style="text-align: center; color: #6b7280; padding: 2rem;">
                                No inventories found in this room.
                            </div>
                        @endforelse
                    </div>
                </div>
